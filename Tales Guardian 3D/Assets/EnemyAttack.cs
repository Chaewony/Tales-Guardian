...
public class EnemyAttack : MonoBehaviour
{
	...
	public void EnemyAttackChoose()
	{
		if (!isChoosingEnd)
		{
			...
			int charRand = Random.Range(0, battleManager.enemySquad.Count); //캐릭터 랜덤
	...
	public void Attack()
	{
		...
		for (int i = 0; i < playerFields[myFieldRand].selectedSecondTarget.Count; i++)
		{
			if (battleManager.playerSquadCharacters[i].transform.position.x == playerFields[myFieldRand].selectedSecondTarget[i].transform.position.x &&
				battleManager.playerSquadCharacters[i].transform.position.z == playerFields[myFieldRand].selectedSecondTarget[i].transform.position.z)
			{
				battleManager.playerSquad[i].myCurrentHp -= 5;
			}
		}
		isChoosingEnd = false;
